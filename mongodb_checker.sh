#!/bin/bash
#########################################################################
# File Name: mongodb_checker.sh
# Author: Carson Wang
# mail: carson.wang@droi.com
# Created Time: 2018-03-23 15:07:52
#########################################################################
...
log_file="mongodb_checker.log"

if pgrep -x "mongod" > /dev/null
then
    echo -e "${now}  ${GREEN}Mongo is running.${NC}" >> mongodb_checker.log
    echo "---------------------------------------------------------" >> mongodb_checker.log
else
    echo -e "${now}  ${RED}Mongo is stopped.${NC}" >> mongodb_checker.log
    echo "---------------------------------------------------------" >> mongodb_checker.log
